...
def load_file(parent_frame):
    ...
    try:
        ...
        APP_DATA["rows"], APP_DATA["cols"] = df.shape
        ...
        if hasattr(root, "show_toast"):
            root.show_toast("Import Successful", f"Loaded {len(df)} rows.")
...
class DataModule:
    ...
    def render_tab(parent, tab_name: str):
        if tab_name == "Import":
            ...
            ttk.Label(
                info,
                text=f"File: {APP_DATA['filename']} | Rows: {APP_DATA['rows']}",
                font=("Segoe UI", 10, "bold"),
            ).pack(side="left")
            ...
            def generate_plot(*args):
                ...
                if series.nunique() < 20:
                    ...
                    series.plot(kind="hist", bins=20, ax=ax, edgecolor="white")
...
class NestedTabApp(tb.Window):
    ...
    def new_project(self):
        self.show_toast("New Project", "Project initialized", bg=TOAST_SUCCESS)
